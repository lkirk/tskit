...
### Two-locus site statistics

The {meth}`~TreeSequence.ld_matrix` method provides access to a collection of
two-locus statistics. These statistics are computed on a site-by-site basis and
are designed to operate on multi-allelic data. Within this framework, we also
implement polarisation for certain statistics requiring polarisation.
...
Generally speaking, we use the `hap_weighted` method of normalisation for
unpolarised ratio statistics, such as {math}`r^2` and {math}`D^{\prime}`.
We use `total_weighted` to normalise the rest.
...
: {math}`f(f_a, f_b, f_{ab}) = \frac{D}{D_{max}}`
  ...
  Where {math}`D_{max}` is defined as:
